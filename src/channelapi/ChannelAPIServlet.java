...
public class ChannelAPIServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		if(txt instanceof String){
			...
			System.out.println("Does "+name+" still exist?: "+updateList.contains(name));
		...
		for(;i<updateList.size();i++){
			try{
				...
				System.out.println("request received from "+name+" and sent to "+updateList.get(i));
				channelService.sendMessage(new ChannelMessage(updateList.get(i),message));
		...
		for(;i<updateList.size();i++){
			try{
				...
				System.out.println("Updating User "+(i+1));
				System.out.println(message);
				System.out.println("request received from "+name+" and sent to "+updateList.get(updateList.indexOf(name)));
				channelService.sendMessage(new ChannelMessage(updateList.get(i),message));
				System.out.println("here");
	...
	try{
		...
		System.out.println(message);
		System.out.println("request received from "+name+" and sent to "+to);
